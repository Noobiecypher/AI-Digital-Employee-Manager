...
----------------
    ...
    POST   /auth/mock-login                  — dev only; issue JWT by role, no password
...
# ---------------------------------------------------------------------------
# DEV ONLY — Mock login by role
# Issues a real JWT for the first active user found with the requested role.
# Disable by setting ALLOW_MOCK_LOGIN=false in your .env file before
...
@auth_router.post(
    "/mock-login",
    response_model=TokenResponse,
    summary="[DEV ONLY] Issue a real JWT for a given role without a password",
    description=(
        "Finds the first active user with the requested role and returns a "
        "signed JWT access token. No password required. "
        "Intended for local development only. "
        "Disable by setting ALLOW_MOCK_LOGIN=false in your .env file."
    ),
)
async def mock_login(body: dict = Body(...)) -> TokenResponse:
    """Dev-only: issue a real JWT for a role without credentials."""

    # Gate: disabled in production via env flag
    if os.getenv("ALLOW_MOCK_LOGIN", "true").lower() != "true":
        raise error_response(
    ...
    valid_roles = {"admin", "manager", "hr", "employee", "candidate"}
    ...
    try:
        ...
        logger.error("mock_login: DB error listing users: %s", exc)
        raise error_response(
            status.HTTP_500_INTERNAL_SERVER_ERROR,
            "INTERNAL_SERVER_ERROR",
    ...
    user = next(
        (u for u in users if u.get("role") == role and u.get("is_active", False)),
        None,
    )
    ...
    if not user:
        raise error_response(
            ...
            "ENTITY_NOT_FOUND",
            (
                f"No active user found with role '{role}'. "
                f"Create one via POST /auth/users first, or run the seed script."
            ),
        )
