...
export default function EmployeeList() {
  ...
  const columns = [
    ...
    { key: 'employee_name', label: 'Name', render: v => <span style={{ fontWeight: 500, color: 'var(--color-text-primary)' }}>{v}</span> },
    { key: 'role',          label: 'Role' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <PageHeader title="Employees" count={filtered.length} search={search} onSearch={setSearch} onAdd={() => navigate('/employees/new')} addLabel="Add Employee" />
      <DataTable columns={columns} data={filtered} loading={loading} emptyMessage="No employees found" />
      <ConfirmDialog isOpen={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Remove Employee" message={`Are you sure you want to remove ${deleteTarget?.employee_name}? This action cannot be undone.`} />
    </div>
  )
}
...
function WorkModeBadge({ mode }) {
  ...
  return (
    <span style={{ padding: '2px 9px', borderRadius: 12, fontSize: 12, fontWeight: 500, textTransform: 'capitalize', background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>
      {mode || '—'}
    </span>
  )
}
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
      <div style={{ flex: 1 }}>
        ...
        {count != null && <p style={{ fontSize: 12, color: 'var(--color-text-muted)', marginTop: 2 }}>{count} record{count !== 1 ? 's' : ''}</p>}
      </div>
      {onSearch && (
